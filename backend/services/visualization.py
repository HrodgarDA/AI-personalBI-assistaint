...
def plot_category_totals(data: List[Dict[str, Any]]):
    """Generates a horizontal bar chart for category totals."""
    theme = DARK_THEME
...
def plot_category_pie(data: List[Dict[str, Any]]):
    """Generates a donut chart for category distribution."""
    theme = DARK_THEME
    totals = defaultdict(float)
    for row in data:
        amount = row.get("amount")
...
def plot_amount_over_time(data: List[Dict[str, Any]], freq="D", cumulative=True):
    ...
    if has_incoming:
        fig.add_trace(go.Scatter(
            x=x_labels, y=final_incoming, name=f"{label_prefix} Income",
            mode="lines+markers", line=dict(color=theme["palette"][1], width=3, shape='spline'),
            fill='tozeroy', fillcolor='rgba(52, 211, 153, 0.1)',
            hovertemplate=f"<b>%{{x}}</b><br>{label_prefix} Income: %{{y:.2f}} €{mode_suffix}<extra></extra>"
        ))
        
    fig.add_trace(go.Scatter(
        x=x_labels, y=final_outgoing, name=f"{label_prefix} Expenses",
        mode="lines+markers", line=dict(color=theme["palette"][3], width=3, shape='spline'),
        fill='tozeroy', fillcolor='rgba(248, 113, 113, 0.1)',
        hovertemplate=f"<b>%{{x}}</b><br>{label_prefix} Expenses: %{{y:.2f}} €{mode_suffix}<extra></extra>"
    ))
    
    chart_title = "Cumulative Spending Trend" if cumulative else "Periodical Spending Trend"
    fig.update_layout(

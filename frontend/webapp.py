...
# Ensure local 'backend' directory is available
...
def main():
    st.set_page_config(page_title="AI Personal BI Assistant", page_icon="📈", layout="wide")
    ...
    if st.session_state["current_page"] == "Settings":
        ...
        if not data:
            st.info("Welcome to your AI Personal BI Assistant! Start by uploading your bank statement in the sidebar.")
            st.stop()
            
        st.markdown("<h1 style='text-align: center;'>AI Personal BI Assistant</h1>", unsafe_allow_html=True)
        
        # 1. Filter Logic & State Orchestration
        with st.container(border=True):
            tx_types = sorted({str(row.get("transaction_type", "")) for row in data if row.get("transaction_type")})
            tx_types.insert(0, "All")
            ...
            t_index = tx_types.index(st.session_state.get("selected_tx_type", "All")) if st.session_state.get("selected_tx_type") in tx_types else 0
            selected_tx_type = col2.selectbox("Transaction Type", tx_types, index=t_index, key="selected_tx_type", on_change=sync_url_from_state)
            
            # Categories (Dynamic list via Service)
            available_categories = fetch_categories(selected_tx_type)

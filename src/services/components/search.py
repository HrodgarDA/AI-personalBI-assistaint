...
from src.utils.utils import clean_search_query, is_valid_search_query
from src.utils.usage_manager import SearchUsageManager
...
class SearchEngine:
    ...
    def __init__(self, backends: List[str], timeout: int):
        ...
        self.usage_manager = SearchUsageManager("data/search_usage.json")
        self.tavily_api_key = os.getenv("TAVILY_API_KEY")
        logger.info(f"🔎 [SEARCH-INIT] Engine ready. Monthly quota: {self.usage_manager.get_status_str()}")
    ...
    def search_merchant_info(self, merchant_name: str) -> str:
        ...
        if not is_valid_search_query(query):
            logger.info(f"   ⏩ [SKIP] Skipping web search for '{merchant_name}' (Likely person or too short).")
            return ""
        ...
        logger.info(f"   ⚠️ [WEB] No information found for '{merchant_name}' across all backends.")
        return ""

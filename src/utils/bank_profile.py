...
class BankProfile(BaseModel):
    ...
    historical_avg_period: int = 6
    
    @classmethod
    def from_config(cls, name: str, config_json: str) -> "BankProfile":
        if not config_json:
...
async def load_active_profile_from_db(db_session) -> Optional[BankProfile]:
    from src.database.models import BankProfile as DBBankProfile
    from sqlalchemy.future import select
    
    result = await db_session.execute(select(DBBankProfile).filter(DBBankProfile.is_active == True))
    db_profile = result.scalar_one_or_none()
    
    if db_profile:
        return BankProfile.from_config(db_profile.name, db_profile.config)
    return None
